Starting point for `index.tsx` in `app/routes`.
...
function Home() {
  return <div className="flex flex-col py-10 px-10 text-2xl"></div>;
}
```
...
<form onSubmit={handleSubmit} className="fixed bottom-0 w-full max-w-3/4">
  <Input
    className="w-full"
    value={input}
